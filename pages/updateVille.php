...
$update_data = Get_Modify_ville($conx, $_GET["id"]);
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <input type="text" name="ville_nom" value="<?php echo $update_data['nom']; ?>"
                                    class="form-control" placeholder="Nom de la Ville" required>
                            ...
                            <div class="col-md-6">
                                <select name="ville_pays" class="form-select" id="pays">
                                    ...
                                    <?php foreach ($pays as $item): ?>
                                        <option value="<?= $item['id_pays'] ?>" <?php if ($update_data['id_pays'] == $item['id_pays'])
                                              echo 'selected'; ?>>
                                            <?= $item['nom'] ?>
                                        </option>
                                    <?php endforeach; ?>
                            ...
                            <div class="col-12">
                                <select name="ville_type" class="form-select" required>
                                    ...
                                    <option value="town" <?php if ($update_data['type'] == 'town')
                                        echo 'selected'; ?>>
                                        Town</option>
                                    <option value="capital" <?php if ($update_data['type'] == 'capital')
                                        echo 'selected'; ?>>capital</option>
                                </select>
                            </div>
                            <div class="col-12 text-center">
                                <button type="submit" class="btn btn-primary rounded py-3 px-5">Modifier</button>
                            </div>
                        </div>
                    </form>
                </div>
